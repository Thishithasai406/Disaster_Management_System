...
async def create_incident(
    ...
    severity_map = {"low": Severity.LOW, "medium": Severity.MEDIUM, "high": Severity.CRITICAL, "critical": Severity.CRITICAL}
    incident_severity = severity_map.get(incident.severity, Severity.MEDIUM)
    
    await notify(
        event_key=f"incident_{incident.id}",
...
async def verify_incident(
    ...
    try:
        ...
        # Send WebSocket notification
        await notify(
            event_key=f"incident_verified_{incident.id}",
            title=f"Incident Verified: {incident.title}",
            message=f"The incident has been verified by admin",
...
async def start_incident(
    ...
    try:
        ...
        # Send WebSocket notification
        await notify(
            event_key=f"incident_started_{incident.id}",
            title=f"Incident In Progress: {incident.title}",
            message=f"Work has started on this incident",
            severity=Severity.MEDIUM
        )
...
async def resolve_incident(
    ...
    try:
        ...
        # Send WebSocket notification
        await notify(
            event_key=f"incident_resolved_{incident.id}",
            title=f"Incident Resolved: {incident.title}",
            message=f"The incident has been marked as resolved",
