    ...
    navigator.geolocation.getCurrentPosition(
      async ({ coords }) => {
        ...
        try {
          ...
          setGpsAddress(parts.join(', ') || json.display_name);
        } catch {
          setGpsAddress(null);
        } finally {
          setGpsLoading(false);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2.5rem' }}>
          ...
          <div className="card" style={{ background: 'rgba(99,102,241,0.05)', borderColor: 'rgba(99,102,241,0.15)', padding: '1.5rem' }}>
            <div className="flex items-center justify-between" style={{ marginBottom: '1rem' }}>
              ...
              {!gpsError && !gpsLoading && gpsAddress && (
                <div className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                  <span className="text-[10px] font-semibold text-green-400 uppercase tracking-wide">Live GPS</span>
                </div>
              )}
            </div>
            ...
            {gpsLoading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <div className="skeleton h-3 w-1/2 rounded" />
                <p className="text-xs text-slate-600 mt-1">Acquiring GPS signal…</p>
              </div>
            ) : gpsAddress ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                <p className="text-sm text-slate-200 font-semibold leading-snug">{gpsAddress}</p>
                {gpsCoords && (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                ...
                <p className="text-xs text-slate-500">{showAddress ? address : 'GPS unavailable — enable location access'}</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
                <p className="text-sm text-slate-300 font-medium">{user?.city || 'Location not set'}</p>
